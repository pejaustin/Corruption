...
**Build status:** First-pass MVP shipped (handoff intake + courier dispatch only). Stats at `data/minions/advisor.tres`. Actor scene `scenes/actors/minion/types/advisor_actor.tscn` runs two custom states — `advisor_idle_state.gd` and `advisor_follow_state.gd` (mounted as overrides on the inherited IdleState / ChaseState nodes; both find the owner overlord via the `actors` group and the `owner_peer_id`-named OverlordActor). The Advisor follows its owner with a 2.0m / 3.5m hysteresis. A `HandoffArea` Area3D child (1.2×3×1.2 box centered at body height, on the interactable layer so the gaze raycast detects it) runs `scripts/interactibles/advisor_handoff.gd`: when the owning overlord aims at the Advisor and presses E, `KnowledgeManager.request_dispatch` gathers the entries currently in `&"readied"` stage from the owner's **local** `WorldModel.pending_commands` and ships them to the host (owner-authoritative handoff, fixed 2026-06-05 — models are never replicated, so the host can't read a client's entries). The host batches the sub-orders into couriers per `MinionType.max_orders`, spawns them at the tower's `CourierSpawn` with the delivery payload (`delivery_legs`, `return_pos`, `return_zone`) attached, and confirms each entry back to the owner — the local entry flips to `&"dispatched"` with `courier_id` + `route_waypoints` filled in. Pre-placed tower Advisors are adopted into `MinionManager._minions_node` at scene setup so their follow movement syncs to every peer. What's still TODO: paper-item handoff (`held-items.md`), spoken/balcony intake mode, multi-command-type parsing (currently movement only), faction courier caps, Stay-vs-Leave delivery modes, dominate/impersonate hooks, and combat-survivability tuning (currently hp 40, no aggro, no defenses).
