...
| **Advisor** (`AdvisorHandoff`) | on the advisor minion (separate from the table) | promote every readied → dispatched (`KnowledgeManager.request_dispatch` → host spawns couriers → per-entry confirm) |
...
When the overlord interacts with the **Advisor**, the handoff is **owner-authoritative** (fixed 2026-06-05 — pending_commands live only in the owner's local model, which the host can't read): `KnowledgeManager.request_dispatch` gathers the local readied entries and ships them to the host via `_request_dispatch_rpc` (sender identity = owner; payload never trusted for it). The host (`_dispatch_entries`) batches the sub-orders into couriers, spawns them at the owner's tower spawn marker with the delivery payload (`delivery_legs`, `return_pos`, `return_zone`) attached, and answers every entry via `_dispatch_confirmed_rpc` — the owner's local entry flips `stage` to `&"dispatched"` with `courier_id` + `route_waypoints`, or just clears its in-flight latch on rejection (courier spawn failed; entry stays readied for retry). Delivery-failure reports take the same hop home (`notify_delivery_failures` → `_delivery_failures_rpc` to the owner).
...
| `dispatched` | Advisor E (`KnowledgeManager.request_dispatch` → host confirm) | Same arrow flips to **black** in place. | **Blue** courier-color arrow appears — the runner's path from the tower to the believed source. |
...
Support staff — couriers of both kinds plus the Advisor (`KnowledgeManager.UNTRACKED_TRAITS`) — **never enter any WorldModel**, own or rival, so they never render as pieces on any table (design call 2026-06-05; previously rival couriers leaked in as regular sightings and advisors got pawns). Your own couriers exist on the table purely as the order/route arrow visuals; rivals' runners and advisors are invisible — spotting them is future interception gameplay (step 14 falsification territory), not a free sighting from the broadcast loop. The reality overlay still keys on `COURIER_TRAITS` — `M` shows ground-truth spheres for couriers only.
...
- `KnowledgeManager.request_dispatch` → `_dispatch_entries` (host):
  - Owner gathers its local readied entries (in-flight latch against double-E) and ships `{cmd_id, target_pos, legs}` to the host; sender identity is the owner.
  - Host flattens every entry's `legs` into a list of sub-orders `{source_pos, minion_ids, target_pos, cmd_id}`.
  ...
  - Spawns one courier per batch with `courier.delivery_legs = ordered_legs`. Every requested entry gets exactly one `_dispatch_confirmed_rpc` back to the owner — promoted to dispatched with the courier's id + shared `route_waypoints` polyline (renderer dedups by `courier_id`), or rejected (-1) so it stays readied for retry.
